...
def fetch_product(product, timeout=15):
    ...
    try:
        ...
        raise EOLDAPIError(f"Network or API error while requesting {url}: {e}")
    ...
    try:
        ...
        raise EOLDAPIError(f"Invalid JSON received from API: {e}")
...
def save_json(data, path):
    ...
    try:
        ...
        raise FileSaveError(f"Failed to write file '{path}': {e}")


def parse_args():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(
        description="Fetch end-of-life data for a product from endoflife.date API and save as JSON."
    )
    parser.add_argument(
        "product",
        help="Product slug (e.g., python, ubuntu, nodejs)"
    )
    parser.add_argument(
        "-o", "--output",
